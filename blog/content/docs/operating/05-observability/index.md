...
**What normal looks like under load (measured 2026-09-14).** Two soaks of 7500 rerank requests, generated on mini-1 against the retrieval tier, ran at about 16.5 requests per second each. That is the server's ceiling: tripling the client concurrency only lengthened the queue. Each ran for roughly seven and a half minutes. Neither moved anything that matters. There were zero leader changes on all three members, `etcd_server_has_leader` stayed at 1, WAL fsync p99 on the loaded node went from 2.0 to 3.5 ms (peers flat near 2.0 ms, against a 50 ms alert threshold), and cluster apiserver p99 went from 22–24 to about 24 ms. One-minute p99 samples are noisy. An idle sample has shown one mini at 15 ms fsync, and another at 44 ms apiserver, on its own. So compare a node against its own baseline, never against a peer's single reading.

The curated board is the only etcd dashboard. While the chart's `kubeEtcd` block was enabled, it also rendered a board titled **`etcd`** and a `VMRule` of 15 upstream etcd alerts, which never evaluated because `vmalert` is disabled here. Both went away with `kubeEtcd.enabled: false`, and their live copies were deleted on 2026-09-14 (manual op `obs-etcd-chart-orphans-delete`), because the Application runs `prune: false`. If a board titled `etcd` ever reappears, it is an orphan: delete it, never the Frank board. The live rules are the six `layer-2-etcd-*` ones below.
...
| **etcd Scrape Absent** | no (bridge) | `absent(up{job="kube-etcd"})` — the scrape produced *no series at all* for 15m. Every other etcd rule is now blind and reading OK. | Read vmagent's target list (the `kube-etcd` rows above). None means the `VMStaticScrape` is missing; also check the Application for an `ExcludedResourceWarning`. Do not look for an `Endpoints` object. |
