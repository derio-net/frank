...
The prompt text in `blog/prompt_for_images.yaml` is verbose deliberately — the character-fidelity block ("no nose," "visible Frankenstein stitches," "RJ45-connector neck bolts," "messy black hair flat-top") is the same character sheet used on every cover, copy-pasted into each prompt entry so Gemini doesn't drift. A papers-specific master reference at `.reference-pool/papers/reference-papers.png` is passed in on every Papers generation too, with the explicit instruction to treat it as a strict character sheet for face shape and stitches. (Until 2026-05-23 this was a single `blog/static/images/reference.png` shared across all series; it's now per-series — see the [reference-pool refactor PR](https://github.com/derio-net/frank/pull/380).)

The assets get generated by:

```bash
.venv/bin/python scripts/generate-all-images.py --only banner-papers
```
