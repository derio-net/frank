...
The same table would send other alias families somewhere else entirely: `claude` → `anthropic`, `gpt-4` / `o1-` / `o3-` → `openai-codex`, `gpt-5` → `copilot`, `hermes-` → `nous`, `glm-` → `zai`, `kimi` → `kimi-coding`. A LiteLLM alias that starts with one of those prefixes gets a forced `--provider` and bypasses the gateway. `qwen`, `mistral`, `llama`, `deepseek` and `gemini` map to `auto`; `gemma` matches nothing and also falls through to `auto`. Name local aliases accordingly.
...
- **Wire configuration to the binary that actually runs, not to a version you pinned once.** The image-baked opencode and the PVC copy swapped places as the newer one within four months. Config keyed to `PATH` resolution survived that; config keyed to a path would not have.
