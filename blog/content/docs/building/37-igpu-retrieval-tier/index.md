...
**And the memory never comes back — because it is pinned GPU memory, not process memory.** The iGPU has no VRAM, so every allocation OpenVINO makes on it is a shmem-backed host page, *unevictable* inside the container's cgroup. With the server holding 5.11 GiB, `memory.stat` reads `shmem` 5.11 GiB, `unevictable` 5.11 GiB — the same number — `inactive_file` and `active_file` both zero, `anon` 0.55 GiB, and swap disabled. There is no page cache to drop and nowhere to swap, so the kernel may reclaim *nothing* and kills instead. `limits.memory` on this container is not a margin around a process: it is the GPU's memory budget. Baseline after load is 1.71 GiB — the weights, pinned on the GPU — and the pool grows from there toward the largest request shape served, never shrinking.

That is half the bug, and it is the half the report could not see. Take the largest batch this endpoint is expected to serve: it costs 2.93 GiB, so on a freshly restarted pod it totals 5.14 GiB and fits inside the 6 GiB limit with room to spare. It dies only once the floor has risen underneath it. A reranker that works, then doesn't, then works again after a restart is not a request-size problem, it is a ratchet — and a ratchet explains 1 → 9 restarts across a week of light use, which no single request size does. It also means restarting the pod was a real mitigation the whole time, not a superstition — a restart is the only operation that returns the pool to baseline. So the cap and the restart are not alternatives: the cap bounds what one request may allocate, and only a restart bounds what a long-lived server accumulates. Both ship.

One more thing worth saying plainly, because it cost me three wrong explanations in a row: **none of the obvious metrics can see this.** `memory.current` counts pinned shmem *and* page cache, so a high-water reading looks like a leak. `container_memory_working_set_bytes` cannot separate them either. And `container_memory_rss` counts neither — it sat flat at ~0.45 GiB on busy days and idle days alike, which reads as proof that nothing is accumulating. Only `/sys/fs/cgroup/memory.stat`, read inside the container, distinguishes them.
