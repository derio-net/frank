...
The cluster can reason ([Layer 11]({{< relref "/docs/building/11-agentic-control-plane" >}})), orchestrate ([Layer 15]({{< relref "/docs/building/15-paperclip" >}})), and generate media ([Layer 16]({{< relref "/docs/building/16-media-generation" >}})). But most real work isn't a single model call — it's a chain of steps: fetch data from an API, transform it, call an LLM, post the result somewhere, repeat on a schedule. That's workflow automation.
