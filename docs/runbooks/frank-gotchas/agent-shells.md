...
**PID-1 caveat (live-verified 2026-07-24, the post-#687 smoke test): the `/proc/1/environ` escape hatch requires a non-sshd PID 1.** In the hermes `ssh` sidecar, PID 1 is `sshd` itself (the `hermes-agent-shell-ssh` image has no s6/init), and OpenSSH overwrites its argv/environ region with its process title — `/proc/1/environ` returns `sshd: … [listener] 0 of 10-100 startups`, not the env. Any shim reading it gets nothing. `secure-agent-pod`'s `kali` is unaffected (s6 `/init` is PID 1 and preserves environ; login-shell re-export live-proven with `env -i bash -lc`). Consequence: the sidecar's **BYOK re-export (`OPENAI_*`) is dead too** — a secret can't be hardcoded the way `FR_ISOLATION_TARGET` was — tracked in frank#688 (env-dump-before-exec wrapper or an image init are the fix candidates).
