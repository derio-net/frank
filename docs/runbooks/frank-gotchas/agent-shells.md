...
- **hermes** — the existing `apps/hermes-agent-shell/manifests/configmap-byok-env.yaml` (`35-hermes-agent-shell-byok-env.sh`) already re-exports the BYOK vars; `FR_ISOLATION_TARGET` was added to its loop list (`OPENAI_BASE_URL OPENAI_API_KEY FR_ISOLATION_TARGET`). No new mount. **BUT the environ loop is vacuous in this sidecar** (see the PID-1 caveat below) — the var is actually delivered by a static `export FR_ISOLATION_TARGET=worktree` fallback appended to the shim (fine: it's static config, not a secret; the guard test pins the literal to the Deployment value).
...
**PID-1 caveat (live-verified 2026-07-24, the post-#687 smoke test): the `/proc/1/environ` escape hatch requires a non-sshd PID 1.** In the hermes `ssh` sidecar, PID 1 is `sshd` itself (the `hermes-agent-shell-ssh` image has no s6/init), and OpenSSH overwrites its argv/environ region with its process title — `/proc/1/environ` returns `sshd: … [listener] 0 of 10-100 startups`, not the env. Any shim reading it gets nothing. `secure-agent-pod`'s `kali` is unaffected (s6 `/init` is PID 1 and preserves environ; login-shell re-export live-proven with `env -i bash -lc`).

**BYOK secret re-export — FIXED via an env-snapshot wrapper (frank#688).** The static `FR_ISOLATION_TARGET` got a literal fallback (above), but the dynamic BYOK secrets (`OPENAI_BASE_URL`/`OPENAI_API_KEY`) can't be hardcoded. Fix (frank-side, no image rebuild — issue candidate 1): the `ssh` container overrides its `command` to snapshot those secrets from the container env — which the wrapper process *does* have, before it hands off — into `/run/hermes-env/byok` (an `emptyDir` `medium: Memory` tmpfs, `0600` via `umask 077`, so the secrets stay in RAM), then `exec`s the image entrypoint (`/usr/local/bin/hermes-ssh-sidecar-entrypoint.sh` — NOT raw sshd, or its host-key + authorized_keys first-boot prep would be skipped). The `35-…-byok-env.sh` shim reads that file as a fallback after `/proc/1/environ` (which still works on s6 hosts like `kali`). The command-wrapper path and the shim read-path MUST stay equal — both are pinned by `scripts/tests/test_hermes_ssh_byok_env_snapshot.py`, alongside the memory-emptyDir mount. Verify live: `kubectl exec … -c ssh -- bash -lc 'echo ${OPENAI_BASE_URL:-MISSING}'` (login-shell path) resolves, and `cat /run/hermes-env/byok` shows the two `0600` lines. The agent-images-side alternative (candidate 2: give the sidecar a `dumb-init`/s6 PID 1 so `/proc/1/environ` survives) was deferred — the frank-side wrapper needs no cross-repo image build+bump and matches the sibling #689 frank-side precedent.
