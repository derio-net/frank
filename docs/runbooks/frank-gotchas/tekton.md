...
**Why the fix originally lived in the EventListener and not the Pipeline:** at incident time, `apps/root/templates/tekton-extras.yaml` still carried array-item `jqPathExpressions` on `Pipeline` (`.spec.tasks[]?…`), which under `RespectIgnoreDifferences=true` froze the whole `.spec.tasks` array — an edit to the bridge's task params would have been silently discarded while ArgoCD reported Synced (see the ArgoCD section; frank#664). That Pipeline freeze was removed on 2026-07-30 by declaring the API defaults in git, so Pipeline task-array updates now apply normally too.
...
## The git resolver's feature flag is in another namespace (2026-07-26)

**Symptom.** You are asked to confirm `enable-git-resolver` is on. You run the
obvious command and get nothing:

```console
$ kubectl -n tekton-pipelines get cm resolvers-feature-flags
Error from server (NotFound): configmaps "resolvers-feature-flags" not found
```

`kubectl -n tekton-pipelines get deploy` reinforces it — there is no
`tekton-pipelines-remote-resolvers` among the ten deployments. The natural
conclusion is that Frank installed Tekton without resolver support, which would
make any `pipelineRef.resolver: git` design inert.

**Cause.** Tekton's `release.yaml` puts the remote resolvers in their **own
namespace**, `tekton-pipelines-resolvers`. Frank vendors the full release, so
they are installed — just not where the flag for everything else lives.

```console
$ kubectl -n tekton-pipelines-resolvers get cm resolvers-feature-flags -o jsonpath='{.data}'
{"enable-bundles-resolver":"true","enable-cluster-resolver":"true",
 "enable-git-resolver":"true","enable-hub-resolver":"true"}
```

**The real constraint, once you know it works.** The git resolver has two
modes, and only one of them can authenticate:

- **URL mode** (`url` + `revision` + `pathInRepo`) does a plain anonymous
  clone. There is no token parameter. Embedding credentials in the URL is the
  only option, and a TriggerTemplate cannot inject a Secret into a param value.
- **SCM-API mode** (`org` + `repo` + `pathInRepo` + `token`) authenticates, but
  `scm-type` and `server-url` are **global** in `git-resolver-config` — changing
  them affects every resolver consumer on the cluster.

So a pipeline resolved from a private Gitea repo in URL mode cannot work. For
`derio-homelab/kid-laptops` the mirror repo is created **public inside Gitea**
(LAN-only, no ingress, no public DNS) rather than switching a cluster-global
setting for one consumer. The trade-off is real and deliberate: a repo private
on GitHub is readable by anything on the LAN that can reach Gitea.

## The generic gitea-push trigger is an exclusion list, so new triggers double-fire (2026-07-26)

**Symptom.** A repo gets its own trigger on `gitea-listener`. Its pipeline runs
correctly — and a *second*, failing `gitea-ci-*` PipelineRun appears on every
push. The failure is in a pipeline nobody wired for this repo, under a generic
name, so it reads as flaky shared CI rather than a routing bug.

**Cause.** `gitea-listener`'s catch-all `gitea-push` trigger does not select
repos, it *subtracts* them:

```yaml
value: >-
  header.match('X-Gitea-Event', 'push') &&
  !body.repository.full_name.startsWith('agentic-stoa/') &&
  !body.repository.full_name.startsWith('derio-homelab/')
```

Adding a dedicated trigger is therefore **additive**. Both match the same event
and Tekton Triggers fires every matching trigger — that is the design, not a
bug. The generic pipeline then runs `gitea-ci` against a repo that has none of
the things it expects.

**Rule.** Every repo that gains a dedicated trigger must be subtracted from the
generic filter in the same commit. `scripts/tests/test_kid_laptops_pipelinerun_contract.py`
asserts the exclusion list covers every prefix with a dedicated trigger.

## `$(uid)` is random, not unique (2026-07-26)

Tekton Triggers exposes `$(uid)` to resource templates. The documentation says
it carries *"a random value, just like the postfix generated by the Kubernetes
`generateName` metadata field"*.

Read that precisely: **random, not unique**. `generateName` gets away with it
because the API server retries on conflict — a template parameter has no such
loop. It is a short random string, and short random strings collide.

That is fine when the value only has to disambiguate an object name. It is not
fine when something downstream derives a *shared resource* from it. kid-laptops
builds its VirtualMachineInstance name from `run-id`, so two runs sharing one
would create and destroy each other's VM — a 4 GiB VM vanishing mid-converge,
reported as an unrelated Ansible connection failure.

**Rule.** Concatenate something deterministic. `run-id` is
`$(tt.params.short-sha)-$(uid)`: a collision now needs two concurrent runs on
the *same commit* that also collide on `$(uid)`, and a stray VMI is greppable
back to a commit.
