...
## Differences from Phase 04 (Nvidia)

| Aspect | Nvidia (Phase 04) | Intel Arc (Phase 05) |
|--------|-------------------|----------------------|
...
**NOTE:** Per-machine `ExtensionsConfiguration` **overrides** (does NOT merge with) the cluster-wide `400-cluster-iscsi-tools` config. `iscsi-tools` must be included here to avoid dropping it from mini nodes (same reason as phase04's Nvidia patch).
...
- Phase 1 complete (mini nodes labeled)
- Phase 2 complete (Cilium CNI running)
- Phase 3 complete (Longhorn running — not strictly required but keeps cluster stable)
