...
A series of tutorial-style blog posts documenting the construction of the frankocluster — an enterprise-grade, AI-hybrid Kubernetes homelab running Talos Linux across heterogeneous hardware. Posts are capability-centric, grouping related phases into cohesive narratives.
...
### Post 2: Building the Foundation (Bootstrap + Phase 1 + Phase 2)
...
- Node configuration — labels, zones, scheduling strategy (Phase 1)
- Cilium CNI — migrating from Flannel, eBPF kube-proxy replacement, L2 LoadBalancer (Phase 2)
- Gotchas: removing Flannel safely, kube-proxy replacement prerequisites
- Tags: `talos`, `cilium`, `networking`, `bootstrap`

### Post 3: Persistent Storage with Longhorn (Phase 3)
...
### Post 4: GPU Compute — NVIDIA and Intel (Phase 4 + Phase 5)
...
- New cluster phases become new posts (or updates to existing posts)
