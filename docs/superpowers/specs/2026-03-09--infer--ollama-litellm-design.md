...
- **LiteLLM as gateway** — Mature OpenAI-compatible proxy with built-in multi-tenancy primitives (virtual keys, per-key budgets). When the tenant layer (vCluster) lands, tenant isolation is a configuration concern, not an architectural change.
