...
- [x] **Step 1: Expose externally** — done in Phase 2 Task 9 (Traefik IngressRoute) and Phase 2 Task 10 (homepage tile). Confirm by opening the homepage at `https://master.cluster.derio.net` and clicking the ruflo tile.
- [x] **Step 2: Building blog post** — done in Phase 5 Task 2. Confirm post is published and series index updated.
- [x] **Step 3: Operating blog post** — done in Phase 5 Task 3. Confirm post is published and operating series index updated.
- [x] **Step 4: Update README** — done in Phase 5 Task 4. Confirm.
- [x] **Step 5: Sync runbook** — done in Phase 5 Task 5. Confirm `docs/runbooks/manual-operations.yaml` contains both `ruflo-infisical-bootstrap` and `ruflo-authentik-outpost-assign` entries.
- [x] **Step 6: Update plan status** — done in Phase 5 Task 6. Confirm `**Status:** Deployed`.
...
| 2026-05-03 | P6 | **All six post-deploy checklist items verified live.** (1) External exposure: `apps/homepage/manifests/configmap-services.yaml:93,95`, `apps/traefik/manifests/ingressroutes.yaml:417-431`, `apps/authentik-extras/manifests/blueprints-cluster-proxy-providers.yaml:311-326`; `https://ruflo.cluster.derio.net/` redirects to Authentik SSO (200 final), `https://master.cluster.derio.net/` returns 200, `nc 192.168.55.222 22` succeeds. (2/3) Blog posts at `blog/content/docs/building/29-ruflo/` and `blog/content/docs/operating/24-ruflo/`; building series index, operating series index, and the cluster-roadmap shortcode all reference layer 29. (4) `README.md` lines 47, 103-104, 197-198, 253-254 cover Technology Stack, Repository Structure, Service Access, and Current Status. (5) Runbook contains all four `orch-ruflo-*` entries (`authentik-outpost-assign:926`, `infisical-bootstrap:973`, `litellm-virtual-key:1006`, `shell-ssh-keys-bootstrap:1021`) — superset of the two required by Phase 6. (6) Plan header reads `**Status:** Deployed`. **Live state:** ruflo pod `2/2 Running` for 5h17m on mini-1 with image SHA `257790c2…` (post-supercronic-fix per agent-images#54); s6 reports `sshd up 19035s`, `supercronic up 19035s` — no crashloop. **Investigated user-reported "problem":** the firehose of `kex_exchange_identification: Connection closed by remote host` lines in `kubectl logs -c ruflo-shell` is benign probe noise. The container's `livenessProbe` (60s) and `readinessProbe` (30s) are `tcpSocket: port: ssh`, so kubelet opens TCP then disconnects without SSH banner exchange — measured rate is exactly 3 lines/min (= 2/min readiness + 1/min liveness). Real clients reach the auth prompt cleanly (`ssh agent@192.168.55.222` returns `Permission denied (publickey,keyboard-interactive)` from this host). No action needed; documenting here so the next operator on this layer doesn't waste time on the trap. The `rexec line 11: Deprecated option UsePrivilegeSeparation` warnings on each connection are upstream agent-shell-base sshd_config noise — separate from ruflo and out of scope. |
