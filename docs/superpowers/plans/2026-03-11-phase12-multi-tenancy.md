# Phase 12: Multi-tenancy — vCluster Implementation Plan
...
```bash
git add -A
git commit -m "chore(phase12): rename multi-tenancy design to phase12

Was phaseXX (unimplemented). Now phase12 as implementation begins."
```
...
### Task 9: Follow standard phase workflow — Blog, README, Runbook

Per the Standard Phase Workflow in CLAUDE.md:

- [ ] **Step 1: Blog post** — Run `/blog-post` skill. Title: "Phase 12 — Multi-tenancy: Disposable Kubernetes Clusters with vCluster". Update the series index (`blog/content/posts/00-overview/index.md`) and roadmap shortcode (`blog/layouts/shortcodes/cluster-roadmap.html`).
