# Phase 06 OpenRGB Server Regression Fix — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Fix the OpenRGB DaemonSet so LEDs reliably turn off on every cold boot, then merge the fix and its backstory seamlessly into the Phase 6 blog post.

**Architecture:** Replace the two-container pod (init `apply-leds` + main `openrgb-server`) with a single container that runs `openrgb --noautoconnect $OPENRGB_ARGS && sleep infinity`. Verify live before touching the blog post. The fix is a bugfix, not a new phase — the blog post absorbs it as a natural continuation of the Phase 6 story.
...
### Task 3: Update the Phase 6 Blog Post
