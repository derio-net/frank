...
Layer 6 established a Kubernetes DaemonSet that controls the ARGB LEDs on gpu-1 by running
...
The original Layer 6 implementation worked on BIOS F3, where the IT5701 firmware was an
...
by Layer 6) has not published anything beyond `release_0.9`.
...
| Time | Event |
|------|-------|
| Layer 6 setup | Two-container DaemonSet deployed, LEDs off (working, BIOS F3) |
