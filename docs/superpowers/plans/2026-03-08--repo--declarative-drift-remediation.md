...
**Goal:** Fix all declarative drift found in Layers 1–8, establish the manual-operations YAML format in plans and a central runbook, create the `/sync-runbook` skill, and update the layer workflow in CLAUDE.md.
...
```yaml
# manual-operation
id: obs-grafana-victorialogs-migration
layer: obs
app: victoria-metrics
plan: docs/superpowers/plans/2026-03-08--repo--declarative-drift-remediation.md
...
**Step 2: Retroactively add manual-operation block to Backup plan**

Append this block to `docs/superpowers/plans/2026-03-08--backup--longhorn-r2.md`:
...
```bash
git add docs/superpowers/plans/2026-03-08--repo--declarative-drift-remediation.md \
        docs/superpowers/plans/2026-03-08--backup--longhorn-r2.md
...
#
# Central registry of all manual operations required across layers.
...
**Step 1: Add `/sync-runbook` to the Standard Layer Workflow section**
...
3. **Blog** — Write a Hugo blog post documenting the layer
...
2. **Deploy** — Implement the ArgoCD app (values, Application CR, manifests)
3. **Blog** — Write a Hugo blog post documenting the layer
4. **Sync runbook** — Run `/sync-runbook` if the layer plan contains any `# manual-operation` blocks
...
Follow the manual operation `obs-grafana-victorialogs-migration` in `docs/runbooks/manual-operations.yaml`:
...
```bash
git add docs/runbooks/manual-operations.yaml docs/superpowers/plans/2026-03-08--repo--declarative-drift-remediation.md
