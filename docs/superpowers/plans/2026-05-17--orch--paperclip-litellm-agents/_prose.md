...
- **P5.T2.S1 — hermes_local adapter's `VALID_PROVIDERS` whitelist drops `ollama-cloud`**: Plan-time verification (P1.T2.S4) probed the hermes CLI directly with `--provider ollama-cloud --model qwen-think-14b` and confirmed it routes through LiteLLM. It did NOT probe via the `hermes_local` adapter. Surfaced in Phase 5 on the first UI hire: the adapter (`hermes-paperclip-adapter@0.2.0`, `dist/server/execute.js:268`) gates the `provider` adapter-config field through a hardcoded list `[auto, openrouter, nous, openai-codex, zai, kimi-coding, minimax, minimax-cn]`. `ollama-cloud` is rejected and the field is silently dropped; hermes runs without `--provider` and errors with "No LLM API keys found (set ANTHROPIC_API_KEY/OPENROUTER_API_KEY/OPENAI_API_KEY/ZAI_API_KEY)". The schema-driven UI's `extraArgs` text input stores user input as a single-element array `["--provider ollama-cloud"]` (one argv token with embedded space) which hermes argparse rejects, so that fallback is also unusable from the UI. **Fix:** ship a smart wrapper in `paperclip-hermes` ConfigMap (`hermes-wrapper` key) and have the `hermes-init` initContainer install it at `/paperclip/agent-bin/bin/hermes`, replacing the venv symlink from Phase 1. The wrapper injects `--provider ollama-cloud` only when the caller didn't pass one — transparent for both the adapter (which never sets `--provider`) and direct CLI use from `paperclip-shell` (Phase 1 probe style still works). Plan template in 05.yaml updated to omit `provider` and `extraArgs` entirely; runbook updated at `docs/runbooks/frank-gotchas/paperclip-ruflo.md#hermes-wrapper-provider-injection`; hot-file one-liner updated at `agents/rules/frank-gotchas.md`.
