...
- **P5.T2.S1 — hermes_local hire first-attempt failure was a model typo, not a missing `--provider`** (and the wrapper I built to "fix" it was both unnecessary and broken — see PR #296 + revert PR for the false-start record):
  - **Sequence:** operator's first UI hire submitted `model: "litelllm/quen36-a3b-nothin"` (four L's in `litelllm`, plus a typo on the model alias). Hermes saw the unknown `litelllm/` provider prefix on `-m`, couldn't infer a provider, and surfaced `warn·No LLM API keys found in environment / Hint: Set ANTHROPIC_API_KEY, OPENROUTER_API_KEY, OPENAI_API_KEY, or ZAI_API_KEY`. The operator (reasonably) read this as a missing-provider-flag error and tried adding `--provider ollama-cloud` via the UI's `extraArgs` text input; the schema-driven config form (`/app/ui/src/adapters/schema-config-fields.tsx`, `SchemaConfigFields`) stored the value as a single-element array `["--provider ollama-cloud"]` (one argv token with embedded space) rather than the two-element array `["--provider", "ollama-cloud"]` that hermes argparse expects. The custom `hermes-paperclip-adapter/ui/build-config.js#buildHermesConfig` whitespace-splits correctly, but the schema-form path doesn't call it.
  - **What I misdiagnosed:** I read the symptoms as proof that `hermes-paperclip-adapter@0.2.0`'s `VALID_PROVIDERS` whitelist (`[auto, openrouter, nous, openai-codex, zai, kimi-coding, minimax, minimax-cn]` — `ollama-cloud` missing) was silently dropping a needed flag, and built PR #296: a `hermes-wrapper` ConfigMap script + initContainer that installed it at `/paperclip/agent-bin/bin/hermes` (replacing the Phase 1 venv symlink), intending to inject `--provider ollama-cloud` before passing argv through to the venv binary.
  - **Why the wrapper was wrong:** `--provider` is a **subcommand-scoped** flag on hermes (`hermes chat --provider X`), not a top-level option. The wrapper prepended it to the full argv, producing `hermes --provider ollama-cloud chat …` — argparse parses the first positional as the subcommand name and errors with `argument command: invalid choice: 'ollama-cloud'`. Verified after merge in Phase 5 driving.
  - **Why the wrapper was also unnecessary:** the seeded `config.yaml` sets `model: "ollama-cloud/qwen-think-14b"`. Hermes uses the prefix on the default model to pin `ollama-cloud` as the **default provider** for the install. When the adapter then passes `-m qwen-think-14b` (bare), hermes still routes to LiteLLM via `OLLAMA_BASE_URL`/`OLLAMA_API_KEY` from the container env. Verified directly: `kubectl exec deploy/paperclip -c paperclip -- /paperclip/agent-bin/hermes-agent/venv/bin/hermes chat -q "say ack" -Q -m qwen-think-14b -t terminal,file,web --source tool` → session created, no error.
  - **Revert + correct fix:** drop the `hermes-wrapper` ConfigMap key; restore the initContainer to seed `config.yaml` only, and also reconcile `/paperclip/agent-bin/bin/hermes` back to a symlink to the venv binary on every boot (so any PVC that carries the broken wrapper file gets reset). Hire payload stays the just `model: "qwen-think-14b"` + `hermesCommand`. The UI traps (whitelist no-op + `extraArgs` schema-form split) are documented in the runbook as "don't set these fields" rather than worked around.
  - **Lesson for plan-time probes:** Phase 1.T2.S4 verified the hermes CLI worked with `--provider ollama-cloud --model qwen-think-14b`. Phase 1 did NOT verify that the adapter's actual invocation pattern (`chat -q PROMPT -Q -m <bare> -t terminal,file,web --source tool` with NO `--provider`) would route correctly. The plan should have probed via the adapter — through the live `hermes_local` codepath or a stub call — not just the underlying CLI in isolation. Adding that check to the standard playbook for any "adapter-driven CLI" plan would have caught both the model typo's actual failure mode AND the absence of any need for `--provider` injection before any manifest changes.
- **P5.T3.S2 — pod-restart resume blocked by upstream session-ID truncation bug** (tracked at [derio-net/paperclip#1](https://github.com/derio-net/paperclip/issues/1)): every `hermes_local` agent fails on its 2nd heartbeat onward. Surfaced after the post-revert hire of Media Pipeline Engineer (UUID `30f6c830-870d-412d-a216-7bf300d37306`) — first run (`heartbeat_runs.id = 7745ce28`) exited 0 in 32s with model=`qwen-think-14b`, provider=`ollama-cloud` (verified in the verbose hermes log, and confirmed by `litellm-5d7466bfd5-2psn6` access log `POST /v1/chat/completions 200 OK` from `10.244.10.177` = paperclip pod IP). Runs 2 (`9fc55148`) and 3 (`f48157c2`) exited 1 in ~2s with `Session not found`. Two interacting bugs:
  - **Adapter:** `hermes-paperclip-adapter@0.2.0` (verified also in `0.3.0`) `dist/server/execute.js:385` sets `executionResult.sessionDisplayId = parsed.sessionId.slice(0, 16)`. Hermes session IDs are 22 chars (`YYYYMMDD_HHMMSS_<6hex>`), so the display value is the 16-char prefix only.
  - **Paperclip:** `server/src/services/heartbeat.ts → resolveNextSessionState` gives `explicitDisplayId` priority over `deserialized?.sessionId`. The truncated display value ends up in `heartbeat_runs.session_id_after` AND gets fed back to the adapter as `--resume <truncated>` on the next heartbeat. Hermes can't find it; the adapter's stdout regex (`/session[_ ](?:id|saved)[:\s]+([a-zA-Z0-9_-]+)/i`) then mis-captures `from` out of hermes's `Use a session ID from a previous CLI run` error line, and `agent_task_sessions.session_params_json` becomes `{"sessionId":"from"}` — permanently. After the 2 retries, paperclip flags the issue as `Stranded` / `RECOVERY NEEDED`.
  - **Phase 5 verification coverage:** the hire works (P5.T2.S1), the first heartbeat works end-to-end through LiteLLM/Ollama (P5.T2.S2, P5.T3.S1). The pod-restart resume test (P5.T3.S2) is structurally blocked by the same bug as 2nd-heartbeat — there is no path through which a hermes_local agent can resume an existing session ID, since the stored ID is always truncated. Marked as a `[-]` skip on P5.T3.S2 pending upstream fix.
  - **Workaround documented in runbook** (`docs/runbooks/frank-gotchas/paperclip-ruflo.md`): hire with `adapterConfig.persistSession: false`, which makes hermes start fresh on every heartbeat (no `--resume` flag, truncation bug never fires). Loses cross-heartbeat session continuity; acceptable for tool-heavy issue work, bad for long multi-turn conversations.
  - **Recovery for an already-stranded agent:** clear `agent_task_sessions.session_params_json` + `session_display_id` for the affected `(agent_id, task_key)` row (or delete the task entirely). The agent itself is fine; only the per-task session state is poisoned.
