...
Daily readings — `restartCount`, p99 working-set RSS over the prior 24 h, and peak queue depth — are operator-driven; this section is the canonical record. Templates and queries are in Phase 5 Task 1 Step 1 above. Phase 4 Task 2 already wired the metrics surface end-to-end (`vibekanban_max_executions=4`, the cadvisor pipeline fix from Phase 2, and the new Grafana panels), so each day is a single read.
