...
See: `docs/superpowers/specs/2026-03-07--secrets--infisical-eso-design.md`
...
```yaml
# manual-operation
id: secrets-infisical-bootstrap-secrets
layer: secrets
app: infisical
plan: docs/superpowers/plans/2026-03-08--secrets--infisical-eso.md
...
```yaml
# manual-operation
id: secrets-eso-credentials-secret
layer: secrets
...
Frank's Layer 9 crosses that inflection point. The goal: a UI-managed secret store that syncs automatically into Kubernetes, with a clear audit trail and no secret-in-plaintext moment anywhere in the pipeline.
...
- The "manual apply" pattern discovered in Layer 8 is a workflow smell
...
- **SOPS + ArgoCD ServerSideApply**: SOPS-encrypted secrets cannot live in ArgoCD-managed manifest paths. Apply them manually with `sops --decrypt | kubectl apply -f -`. See Layer 8 write-up for details.
