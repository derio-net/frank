...
<!-- fr:journal kind=finding scope=plan id=p6-blackbox-stale-config created=2026-07-26T14:09:50 phase=6 state=fixed -->
### p6-blackbox-stale-config · finding [fixed] · blackbox-exporter served a 15-day-old config; the new probe 400'd until SIGHUP (phase 6)

Third instance of the same class this session, after Caddy and act_runner. ArgoCD applied the ConfigMap and created the VMProbe, and the pod's mounted `/etc/blackbox_exporter` file DID contain `www_content` (directory mount, so kubelet live-updates it) — but the process parses config at boot, and the pod had been up since 2026-07-11. So vmagent began scraping `module=www_content` against a process that had never heard of it: `/probe` returned HTTP 400 and the www series went absent, briefly leaving the site with NO uptime signal at all — strictly worse than the blind spot being fixed. Caught by testing the module against the running process (`wget /probe?module=www_content`) rather than trusting the file on disk. Fixed with `kill -HUP 1`; `probe_success 1` immediately after. Durable fix is a checksum annotation or configMapGenerator so the pod rolls on config change.

<!-- fr:journal kind=discovery scope=plan id=p6-merge-patch-drops-siblings created=2026-07-26T14:09:52 phase=6 -->
### p6-merge-patch-drops-siblings · discovery · kubectl --type=merge on a nested map REPLACES it — dropped prune:false (phase 6)

Suspending selfHeal with `--type=merge -p '{"spec":{"syncPolicy":{"automated":{"selfHeal":false}}}}'` replaced the whole `automated` map, silently dropping `prune: false` and leaving the www Application OutOfSync against its root template after restore. Functionally inert (ArgoCD defaults prune to false) but real declarative drift, and `prune: false` is a repo-wide invariant. Restore the FULL map, not just the field you changed — or the documented suspend/restore recipe leaves the app permanently diverged from git.
