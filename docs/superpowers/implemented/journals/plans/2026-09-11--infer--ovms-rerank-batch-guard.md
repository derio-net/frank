...
<!-- fr:journal kind=finding scope=plan id=ten-gi-arm-blocked created=2026-09-11T12:08:19 phase=3 state=fixed -->
### ten-gi-arm-blocked · finding [fixed] · BLOCKED: the 10Gi arm needs a live Deployment patch, which the permission layer refused (phase 3)

CLOSED 2026-09-14 by Test Plan row 10 against the live post-merge pod. The row FAILED its ~70 percent threshold: an ascending sweep of 20, 40 then 64 documents at 600 tokens peaked at 8.49 GiB, 84.9 percent of the 10Gi limit, with no OOM but 1.5 GiB of headroom. The gap was in WHICH case the original sizing measured — the cap as a single call on a fresh pool is 5.67 GiB (57 percent), while an ascending sequence holds the older pool while allocating the new one. Remedies measured on the same ascent: a 512-token bound peaks at 6.96 GiB, and a 16Gi ceiling puts the shipped 640 bound at 53.1 percent. Operator chose 16Gi plus a watchdog tick of 2 minutes and a critical threshold of 50 percent, since no schedule prevents an ascent inside one tick. The ceiling was never unmeasured after this — it is measured now.
...
<!-- fr:journal kind=finding scope=plan id=ten-gi-ships-unmeasured created=2026-09-11T14:22:01 phase=5 state=fixed -->
### ten-gi-ships-unmeasured · finding [fixed] · OPEN: 10Gi ships without ever having been measured at 10Gi (phase 5)
...
<!-- fr:journal kind=review scope=plan id=review-phase-5 created=2026-09-11T14:24:32 phase=5 -->
### review-phase-5 · review · Phase 5 review — no findings; two executor calls improved on the brief (phase 5)

Reviewed the four phase-5 commits. The ceiling moved to 10Gi with requests left at 2Gi, so the schedulers view of mini-1 is unchanged, and the provenance comment carries the measured basis including an explicit WHAT IS NOT PROVEN paragraph saying the figure was never measured at 10Gi and naming the Test Plan row that closes it. That is the right shape for a number the next operator may have to change. Two calls that improved on my brief. First, finding that the ceiling lives in a SECOND file — cpu-arm-pod.yaml, deliberately outside manifests/ so ArgoCD never sees it, therefore invisible to a grep of the manifests directory — and that leaving the arms with different ceilings would have made the CPU control measure the limit rather than the CPU. Named in no plan, spec or brief; found because a pre-existing tripwire demanded it. Second, declining the assertion I specified: I asked for an equality on model rev 2, which is the identical self-disarming defect the executor had fixed one phase earlier in a neighbouring test, correct for exactly one value of a field whose purpose is to move. It shipped a three-way equality across tag, workflow env and container env plus a floor instead. Accepted as written. No review findings.

<!-- fr:journal kind=discovery scope=plan id=b376b9fcf687 created=2026-09-11T14:33:29 phase=6 -->
### b376b9fcf687 · discovery · The gotcha had to be rewritten around the ratchet, and the first clause patterns were typography-sensitive (phase 6)

Two notes on how T1 was built.

WHAT THE DOCUMENT SAYS. The issue framed this as an oversized request killing the server, and a gotcha recording only the two cap values would have shipped that framing unchallenged. The prose leads instead with the two measured facts: cost near-quadratic in document LENGTH and only linear in COUNT (so max_position_embeddings is the primary control and max_allowed_chunks the secondary one, the reverse of the order they suggest themselves in), and memory never released, so the resident floor ratchets to the high-water of the largest call ever served. Everything an operator does with this follows from the second fact, including the one thing they will reach for first: restarting the pod resets the floor, which is a real mitigation and always was. A test that only demanded the numbers would have blessed the wrong story, so the clause list demands the mechanism words and the ratchet evidence pair (2.21 GiB idle, 4.90 GiB resident after one large call) alongside them.
...
<!-- fr:journal kind=finding scope=plan id=mechanism-is-pinned-igpu-shmem created=2026-09-14T17:43:56 phase=3 state=fixed -->
### mechanism-is-pinned-igpu-shmem · finding [fixed] · RETRACTION + verified mechanism: the memory is pinned iGPU shmem, and nothing about the earlier explanations was right (phase 3)

Measured 2026-09-14 from inside the ovms container. The cgroup breakdown settles what three earlier explanations guessed at: shmem 5.11 GiB, unevictable 5.11 GiB (identical), inactive_file 0, active_file 0, anon 0.55, swap.max 0. The memory is Intel i915 GPU buffers — the iGPU has no VRAM, so GPU allocations are shmem-backed host pages, PINNED UNEVICTABLE inside the container cgroup. limits.memory is therefore the GPU memory budget, and when it is reached the kernel has nothing it is permitted to reclaim, so it OOM-kills. This retracts two earlier claims in this journal and in the spec. (1) memory-is-never-released, argued from memory.current tracking memory.peak — that reading could not distinguish pinned shmem from page cache and was used to build a ratchet narrative. (2) the later correction that said the retained memory was reclaimable page cache and the issue framing was right after all — also wrong, argued from container_memory_rss being flat, which is blind to shmem by construction. Both errors have the same root: measuring proxies (memory.current, working_set, RSS) instead of memory.stat. Verified behaviour, each prediction checked against an observation: baseline after load 1.71 GiB shmem (model weights on the GPU) plus about 0.5 anon; a call allocates buffers sized by request shape, 3.40 GiB for 64 documents by 600 tokens; repeating a served shape grows nothing (three identical calls, pool flat); a NEW but SMALLER shape grows nothing (30x500 after 50x500, flat); an ASCENDING sequence costs more than the largest shape alone (fresh to 50x500 gives 3.49, but 20x500 then 50x500 gives 4.13); nothing is released while the container lives. The model predicts both kills observed today: 50x500 then 60x500 needs 1.71+1.77+2.10+0.5 = 6.08 against a 6 GiB limit, and 60x500 then 64x600 needs about 7.7 — both OOMed, as predicted. The same request 60x500 dies on a warm pool and succeeds on a fresh one, proven in both directions, which is the cleanest statement of the failure mode.

<!-- fr:journal kind=discovery scope=plan id=cold-start-and-device-inventory created=2026-09-14T17:44:01 phase=3 -->
### cold-start-and-device-inventory · discovery · Cold start is 11s, there are 3 iGPUs, the model PVC is RWO, and OVMS metrics are off (phase 3)

Facts gathered while evaluating whether this workload should be transient rather than long-lived. Cold start to Ready is 11 seconds on a warm node (seed skipped because the rev marker matches, IR cache present) — NOT the minutes the deployment comment warns about, which described the first-ever IR compile. The cluster advertises 3 iGPU devices, one per mini, each with capacity.memory 0 (borrows host RAM) and millicores 1k; only ONE ResourceClaim exists cluster-wide, so mini-2 and mini-3 iGPUs are idle and freeing mini-1 device buys nothing today. The model PVC is ReadWriteOnce Longhorn, which is why the Deployment is strategy Recreate and which blocks parallel instances outright unless models are read from the image layer instead. OVMS metrics are NOT enabled: no --metrics_enable in the args and /metrics returns 400, so there is currently no request-rate signal at all and usage can only be inferred from memory graphs — which is exactly how this investigation had to proceed.

<!-- fr:journal kind=decision scope=plan id=watchdog-not-scale-to-zero created=2026-09-14T18:17:18 phase=6 -->
### watchdog-not-scale-to-zero · decision · Pool watchdog (restart) rather than scale-to-zero (phase 6)

Operator asked whether this workload should be transient — a pod per request, or several in parallel — rather than a long-lived server. Evaluated on measurement. OVMS is a serving product with no run-once mode, so per-request instances fight the tool, but the underlying instinct was right: a long-lived process holding unreclaimable GPU buffers is a poor fit for traffic that is idle for days and then bursts. Facts that decided it: cold start to Ready is 11 seconds, against 2-4 seconds of actual rerank work, so per-request pods inflate latency 4-6x; the model PVC is ReadWriteOnce, which is why the Deployment is strategy Recreate and which blocks parallel instances outright unless models are read from the image layer; parallelism tops out at 3 anyway (one iGPU per mini). Transparent scale-to-zero additionally needs an ACTIVATOR in the request path to accept a connection at 0 replicas, scale up, wait out the cold start and proxy — a permanent new component and 11 seconds on the first request of every burst. What it would buy: 2.26 GiB back on a 64 GB node and an iGPU that nothing else claims (one ResourceClaim exists cluster-wide, so two of the three iGPUs are already idle). And it would not address the failure that actually bit, because the measured bursts — seven restarts in three hours on 2026-09-07 — happen WITHIN a session. Chose a watchdog CronJob that restarts on two rules instead: pool at or above 60 percent of the limit restarts even if busy (past that a single cap-sized request cannot fit, so the next caller takes the process down anyway, and an OOM costs the in-flight request plus ten seconds of refused connections where a controlled restart costs eleven), and pool above baseline plus thirty minutes quiet restarts for hygiene. Revisit scale-to-zero if a second consumer appears for the minis iGPUs or host RAM gets tight. Also corrected in the same pass: I had told the operator gpu-switcher was ready-made machinery for this. It is not — it exposes activate and deactivate endpoints for explicit mutual exclusion on gpu-1 single NVIDIA card, with no idle timer and no automatic scale-down.

<!-- fr:journal kind=finding scope=plan id=doc-guards-encoded-the-retracted-mechanism created=2026-09-14T18:17:24 phase=6 state=fixed -->
### doc-guards-encoded-the-retracted-mechanism · finding [fixed] · Three doc guards asserted the retracted mechanism and had to be rewritten, not relaxed (phase 6)

Correcting the prose broke three tests written in phase 5 and 6, which is the guards working — they had pinned the ratchet narrative as required content. test_igpu_dra_docs required the literal words ratchet, high-water or never released in the hot-file one-liner, plus the 2.21 and 4.90 GiB figures taken from memory.current; test_ovms_retrieval_manifests required the same two figures in the manifest comment. All were evidence for a mechanism that has since been retracted. Rewritten to demand the CORRECTED evidence rather than dropped: the cgroup breakdown (unevictable), the measured 1.71 GiB baseline, the warm-versus-fresh proof, and an explicit statement that container_memory_rss is blind to shmem by construction. Net effect is a stronger guard than before, since it now requires the falsifiable observation rather than a narrative word. One pattern also had to be made whitespace-tolerant: the manifest comment is hard-wrapped at about 70 columns, so a multi-word phrase never sits on one line and a literal match would have passed or failed on where a sentence happened to break — the same typography-sensitivity the phase-6 executor hit and fixed once already.

<!-- fr:journal kind=discovery scope=plan id=peak-grows-with-the-limit created=2026-09-16T07:07:35 phase=3 -->
### peak-grows-with-the-limit · discovery · The pinned pool grows when the limit grows — raising the ceiling is not a proportional win (phase 3)

Test Plan row 10 re-run against the deployed 16Gi limit: ascending 20 then 40 then 64 documents at 600 tokens on a fresh pod peaked at 9.59 GiB, 59.9 percent, so the row PASSES its 70 percent threshold. But the prediction was 53.1 percent, derived by assuming the peak is workload-determined and limit-independent. It is not. The SAME ascent measured 8.49 GiB at a 10Gi limit and 9.59 GiB at 16Gi — 13 percent higher for identical requests. The pinned iGPU pool evidently sizes itself partly against available memory, so raising the ceiling partly feeds the thing it is meant to contain and yields less than proportional headroom. One sample, so do not over-model it, but it points the opposite way from just add RAM and it strengthens the note already in the manifest that a THIRD raise should tighten the graph bound instead. Second observation from the same run: settled shmem afterwards is 7.48 GiB while the transient peak was 9.59 GiB, a gap of about 2.1 GiB. The watchdog reads shmem (current occupancy), not memory.peak, so its CRITICAL_PERCENT threshold should be read as a SETTLED-pool figure with roughly 2 GiB of transient on top — at the 50 percent trigger that is about 10 GiB of real exposure against a 16 GiB ceiling, which is still comfortable but is not what the number literally says.

<!-- fr:journal kind=review scope=plan id=test-plan-complete created=2026-09-16T07:07:37 phase=6 -->
### test-plan-complete · review · Test Plan complete: 13 of 13 pass after the 16Gi change (phase 6)

Rows 1 to 9 and 11 to 13 passed on the 10Gi deployment; row 10 failed there at 84.9 percent and drove the 16Gi change, and it passes on the 16Gi deployment at 59.9 percent. Two results worth carrying forward. Row 6 beat the spec prediction — the refusal is HTTP 400 in 7 milliseconds with a body naming the limit, not the 500 the spec asserted from tracing std::runtime_error to absl::InternalError; the source path was traced and the HTTP mapping was assumed. Writing the row to RECORD the observed code rather than assert 4xx is the only reason that landed as good news instead of a failed assertion, and it means issue 793 got the 4xx it actually asked for. Row 12 passed unprompted in production: the hygiene rule stamped the idle clock at 19:25, declined to fire at 300, 600, 901, 1201 and 1500 seconds, restarted at 1800 with reason idle-with-elevated-pool, and the pool returned to baseline — with the Deployment restartedAt annotation matching to the second. That history survives only because every exit path emits a greppable result line; successfulJobsHistoryLimit is 1, so the Job objects are gone within minutes and VictoriaLogs is the only record. The no-running-pod path also fired correctly during the 16Gi rollout, declining to restart a pod that was mid-replacement.
