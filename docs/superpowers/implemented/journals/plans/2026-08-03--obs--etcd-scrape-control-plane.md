...
<!-- fr:journal kind=finding scope=plan id=g1-configpatch-never-restarts-etcd created=2026-09-13T21:48:15 state=fixed -->
### g1-configpatch-never-restarts-etcd · finding [fixed] · BLOCKING: the ConfigPatch never restarts etcd — the pre-merge gate needs a drained rolling reboot

Found 2026-09-13 finishing the PR. ConfigPatch 160-etcd-metrics-listener had been in Omni since 2026-08-03 15:32:14Z and every mini's EtcdSpec carried listen-metrics-urls from that second, yet :2381 was refused on all three 41 days later: etcd 'Started task … 1004h54m ago' predated the spec. Talos v1.12.6's Etcd service implements no APIRestartableService (system.go:469-477 errors 'doesn't support restart operation via API'); siderolabs/talos#9605 closed not-planned. Seven places claimed the apply 'restarts etcd on each control-plane node in turn'; all corrected to a drained rolling reboot, and the gate was then executed: mini-1, mini-3, mini-2 rebooted one at a time with quorum, etcd health and the :2381 listener asserted between nodes.

<!-- fr:journal kind=finding scope=plan id=g2-talosctl-drain-denied-and-cicd-volumes-pin-pdb created=2026-09-13T21:48:24 state=fixed -->
### g2-talosctl-drain-denied-and-cicd-volumes-pin-pdb · finding [fixed] · The documented drain path fails twice on Frank: talosctl --drain is denied via Omni, and detached 1-replica CI volumes pin the Longhorn PDB

(1) talosctl reboot --drain fetches a kubeconfig through the Talos API; Omni's proxy returns PermissionDenied even for the devops SA (Omni role Admin). It fails before cordoning, so harmless, but it is not a drain — use kubectl drain. (2) mini-3 and mini-2 each held six detached longhorn-cicd (numberOfReplicas 1) volumes from kid-laptops-ci PipelineRuns 43-48 days old; under block-if-contains-last-replica the instance-manager PDB never lifts, so the drain stalls for its full timeout. A healthy-attached-volumes pre-flight misses them (detached = robustness unknown). Operator chose to reboot through once only static pods and the instance-manager remained; the leak itself is #791 (commented with the evidence). Both recorded in docs/runbooks/frank-gotchas/omni.md.

<!-- fr:journal kind=discovery scope=plan id=gate-evidence-etcd-listener-open created=2026-09-13T22:45:11 phase=5 -->
### gate-evidence-etcd-listener-open · discovery · Pre-merge gate satisfied: :2381 serves etcd metrics on all three minis after the drained rolling reboot (phase 5)

Captured 2026-09-13T20:43:54Z, after rebooting mini-1 (18:57Z), mini-3 (19:33Z) and mini-2 (20:29Z) one at a time. etcd process age: .21 1h45m, .22 13m, .23 1h09m, all later than the 2026-08-03 EtcdSpec. etcd status: 3 members, one leader (mini-3), raft indices within 2 entries. From the vmagent pod: 192.168.55.21/22/23:2381 each return etcd_server_has_leader 1 and etcd_mvcc_db_total_size_in_bytes ~3.15e8; before the roll all three returned Connection refused. Longhorn: 0 degraded attached volumes. Nodes Ready and uncordoned. Zero NodeShutdown tombstone pods, because the kubectl drain evicted everything first. Restore point: an etcd snapshot (revision 344191551) taken from the leader before the first reboot, kept outside the repo. The manual-op status stays pending: its verify steps 2-4 (up series, metric families, rules Normal) are post-merge.

<!-- fr:journal kind=discovery scope=plan id=postmerge-804-synced-and-adopted created=2026-09-14T22:32:49 phase=5 -->
### postmerge-804-synced-and-adopted · discovery · #804 synced only after a forced sync; ArgoCD adopted the hand-applied VMStaticScrape and the ExcludedResourceWarning cleared (phase 5)

#804 merged 2026-09-14T19:50:57Z (a4bd4ddc). A refresh annotation did not produce a sync operation at the new revision within ~8.5 min (the multi-source stale-revision trap), so an explicit sync operation was forced at 20:04:04Z (syncOptions ServerSideApply + RespectIgnoreDifferences). It ran at 827ae290, a descendant of a4bd4ddc that only bumps the blog image, and finished Succeeded 20:04:13Z. Result: no ExcludedResourceWarning condition; vmstaticscrape/kube-etcd tracking-id = victoria-metrics:operator.victoriametrics.com/VMStaticScrape:monitoring/kube-etcd (adopted, not recreated). The app read OutOfSync only because of 4 PruneSkipped resources, proving kubeEtcd.enabled=false had synced. grafana-alerting auto-synced a4bd4ddc at 19:53:37Z.

<!-- fr:journal kind=discovery scope=plan id=postmerge-chart-orphans-deleted created=2026-09-14T22:33:00 phase=5 -->
### postmerge-chart-orphans-deleted · discovery · obs-etcd-chart-orphans-delete executed; all five objects verified NotFound against a present-object control (phase 5)

2026-09-14T20:13Z, after the disable synced: deleted service + endpoints (kube-system) and vmservicescrape, vmrule and configmap victoria-metrics-victoria-metrics-k8s-stack-{kube-etcd,etcd} (monitoring). The endpoints delete printed no 'deleted' line; the object was already gone with its Service. The first inline verification loop was VACUOUS (a 'set --' inside the loop clobbered the positional args, so every get ran with an empty namespace and kind and printed 'gone'). Re-verified with explicit per-object gets classifying the error text: all five 'NotFound', and the control vmstaticscrape/kube-etcd read present. victoria-metrics then read Synced/Healthy with no conditions.

<!-- fr:journal kind=discovery scope=plan id=postmerge-grafana-reloaded-rules-normal created=2026-09-14T22:33:10 phase=5 -->
### postmerge-grafana-reloaded-rules-normal · discovery · Grafana restarted to reload provisioned rules: six etcd rules loaded with the new runbook, only the curated board remains, up 3x1 (phase 5)

Provisioned alert rules are read at boot, and #804 changed the layer-2-etcd-scrape-absent runbook annotation, so victoria-metrics-grafana was rollout-restarted at 20:13Z. After the restart: /api/v1/provisioning/alert-rules returns all 6 layer-2-etcd-* uids, and scrape-absent's runbook starts 'Check vmagent has kube-etcd targets'; dashboard search for etcd returns only 'Frank Layer 2 — etcd (curated)' uid frank-l2-etcd, with 5 panels; up{job="kube-etcd"} = 1 on .21/.22/.23:2381. Earlier (16:23Z) all six rules read state inactive, health ok. This is P5.T2.S1's evidence.

<!-- fr:journal kind=discovery scope=plan id=soak-2026-09-14-quorum-preserved created=2026-09-14T22:40:29 phase=5 -->
### soak-2026-09-14-quorum-preserved · discovery · Acceptance soak re-run with etcd evidence: quorum preserved under the retrieval tier's maximum rerank load on mini-1 (phase 5)

Measured with a paced loader Job pinned to mini-1 (POST /v3/rerank, 20 candidates, the ovms-retrieval-bench.py payload). Signals came from VMSingle over each soak's last 240s, against a 60s idle baseline taken just before. Soak 1 (16 workers, baseline 20:18:01-20:19:01Z, load 20:20:05-20:27:37Z): 7500/7500 HTTP 200 in 452s = 16.59/s, latency p50 787 / p99 2613 ms. Soak 2 (48 workers, baseline ending 20:30:31Z, load 20:30:41-20:38:16Z): 7500/7500 in 455s = 16.49/s, p50 2227 / p99 10843 ms. Identical throughput at 3x the concurrency, so the server is the bound. etcd leader changes: 0 on every member in all four windows. has_leader min: 1 throughout. WAL fsync p99: mini-1 2.0->3.4 and 2.0->3.5 ms; mini-2 2.0->2.0 and 1.9->2.0; mini-3 15.7->2.0 and 2.0->2.0 (the 15.7 is one noisy idle minute). apiserver p99, WATCH/CONNECT excluded: cluster 23.5->23.8 and 22.2->24.0 ms; .21 23.9->24.1 and 22.3->24.2; .22 24.4->42.1 and 24.4->40.5 (but 43.8 ms in an earlier idle sample, so not load-attributable); .23 22.7->21.0 and 21.6->22.6. mini-1 stayed Ready with no pressure conditions. ovms working set max 2298/2299 MiB of 10 GiB, and the pod never restarted (pool watchdog: busy, no action). Verdict: the claim holds at the load the workload can actually generate. The loaded node shows a small, repeatable fsync p99 rise (~1.5 ms) far below the 50 ms threshold. NOT like-for-like with 2026-08-03 (~31/s).

<!-- fr:journal kind=finding scope=plan id=followup-rerank-throughput-halved created=2026-09-14T22:40:31 phase=5 state=open -->
### followup-rerank-throughput-halved · finding [open] · FOLLOW-UP: ovms rerank throughput on mini-1 is ~16.5 req/s, about half the ~31/s measured 2026-08-03 — unexplained (phase 5)

Found while re-running the acceptance soak, and not investigated here (out of this plan's scope). Two soaks at 16 and 48 client workers both delivered ~16.5/s, with latency growing from p50 787 ms to 2227 ms as concurrency rose, so the retrieval server is saturated at that rate. Building post 37 records 7500 requests in 240 s (~31/s) from a sequential curl loop on 2026-08-03. That implies ~32 ms per request, vs ~800 ms now at 16 concurrent. Candidate cause, unverified: the rerank batch/sequence-length guard and MODELS_REV bump from the #794 OOM work, which would trade throughput for bounded memory. Request payloads may also differ (candidate count and passage length of the original curl loop are not recorded). Next step belongs to the infer layer: measure single-request latency with ovms-retrieval-bench.py against the current server and compare with the 2026-08-02 bench record. Checked against #805/#807 (2026-09-19): those measured MEMORY against the raised ceiling (worst ascent 9.59 GiB = 59.9% of 16Gi) and did not measure request rate, so this follow-up is still open. Two caveats for whoever picks it up: the soak above ran on 2026-09-14 against the OLD 10Gi ceiling and the old watchdog (*/5, 60% trigger), and #805 has since moved those to 16Gi and */2 at 50%. The guard limits named in #807 (max_allowed_chunks 64, max_position_embeddings 640, live in the served graph.pbtxt) remain the first thing to test against the ~32 ms/request implied by the 2026-08-03 curl loop.
