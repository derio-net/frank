...
status: done
done_note: "2026-09-04 — installed `github:garrytan/gbrain#v0.47.9.0` (sha 6bf8db90) into /opt/data/home/.bun/bin/gbrain on the home PVC; `gbrain --version` -> 0.47.9.0 over a LOGIN shell. Test Plan rows 7 AND 8 both PASS: the pod was deliberately deleted and genuinely recreated (…-r26tz -> …-wpmsz) and the CLI still answered, tree still dated Sep 1 — persisted state, not a re-install. So decision 2 holds. The pin is now resolved AT RUN TIME from the newest release rather than hardcoded: upstream cuts several releases a week (v0.46.32 -> v0.47.9 in nine days), so a frozen tag is stale within days while a resolved-then-recorded tag+sha is still immutable. Bun blocked the postinstall (upstream issue 218); harmless here because it only runs `apply-migrations` and no brain is configured. `gbrain init` deliberately NOT run — that is row 9 and the requesting repo owns it."
```
