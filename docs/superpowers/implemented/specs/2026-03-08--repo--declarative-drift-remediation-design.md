...
**Date:** 2026-03-08
**Scope:** Audit and remediate ad-hoc cluster changes across Layers 1–8; establish a canonical format for documenting permanent manual operations.

---

## Problem Statement

Eight layers of homelab work have accumulated two categories of drift from the declarative-only principle:
...
`docs/runbooks/manual-operations.yaml` — single YAML file, sorted by layer:
...
- GitOps: ArgoCD bootstrap (no plan file)
- Obs: Grafana datasource migration (one-time)
- Backup: R2 SOPS secret
...
4. Rewrites the runbook file sorted by layer, then by id
...
The Standard Layer Workflow gains a `/sync-runbook` step:
...
|------|------|------|------------|
| Declarative Drift Remediation Implementation Plan | derio-net/superpowers-for-vk | `2026-03-08--repo--declarative-drift-remediation` | — |
