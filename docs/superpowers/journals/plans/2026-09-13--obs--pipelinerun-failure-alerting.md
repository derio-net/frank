...
<!-- fr:journal kind=finding scope=plan id=p5-uid-length-outage created=2026-09-14T11:51:15 phase=5 state=fixed -->
### p5-uid-length-outage · finding [fixed] · The restart step caught a total Grafana outage that every other gate had passed (phase 5)

Test Plan step P5.T2.S1 (restart Grafana, confirm rules evaluate) found that layer-25-pipeline-idle-stoa-status-bridge is 41 characters against Grafana's 40-char uid limit. Grafana does not skip an over-long rule — provisioning fails as a unit, the container exits non-zero and CrashLoopBackOffs, and every dashboard, datasource and alert rule goes dark. Alerting was down for roughly 35 minutes. Fixed in #801: both idle rules renamed to a layer-25-idle-<pipeline> scheme (32 and 30 chars) plus a document-wide guard, test_no_rule_uid_exceeds_grafanas_forty_character_limit. Two things worth carrying: (1) eleven rule-specific guards, the folder-wide guards INCLUDING one that asserts uids are non-empty and globally unique, fr plan self-review, four rounds of phase review and a green CI run all passed a 41-character string — every one asks whether a rule is well-FORMED, and the limit is Grafana's, external to the artifact, so only counting characters reaches it; (2) it also passed ArgoCD, which reported Synced/Healthy with a correct ConfigMap on the cluster, because Grafana parses provisioning at BOOT — a synced ConfigMap proves nothing about whether Grafana can start with it, and the restart IS the test. Recovery note: rolling the Deployment back does not help, since the ConfigMap and Deployment are separate objects and the rollback re-mounts the same broken map.

<!-- fr:journal kind=discovery scope=plan id=p5-live-verification-results created=2026-09-14T11:51:17 phase=5 -->
### p5-live-verification-results · discovery · Post-merge verification: scrape, drop and all three rule queries confirmed against live data (phase 5)

vmagent target health=up, lastError empty. VMSingle holds 66 tekton metric names including pipelinerun_duration_seconds_count with a pipeline label; taskruns_pod_latency_milliseconds is ABSENT, so the cardinality drop applied. 689 tekton series against the census prediction of 851 — lower because the controller restarted ~12h earlier and has seen fewer distinct taskruns since, not because the scrape is partial. All three rules' exact refId-A expressions were run against live VMSingle: the failure-ratio query returns status=success with no offending pipelines (parses correctly, quiet because nothing is failing), and the two idle queries return 4 and 2 — non-zero, which rules out the disarmed-dead-man state where a rule is quiet because it reads 0 rather than because traffic is healthy. The predicted deploy-time false positive on github-pull-sync did NOT occur: it happened to run twice inside the window after the scrape went live. That was a coin flip landing well, not a correction to the analysis — the prediction stands for future deploys. Still outstanding: P5.T2.S2, the synthetic always-failing PipelineRun, which is the only proof that the alert FIRES; script prepared for the operator.
