...
<!-- fr:journal kind=decision scope=plan id=p10-11-app-regex-deviation created=2026-09-20T10:41:06 phase=10 -->
...
<!-- fr:journal kind=discovery scope=plan id=p10-review-final-suite created=2026-09-20T11:00:34 phase=10 -->
### p10-review-final-suite · discovery · P10 review fix-pass final verification: 1032 passed, 1 xfailed (phase 10)

Full uv run --group dev pytest scripts/tests -q: 1032 passed, 1 xfailed in 306.57s (clean re-run). Baseline at the start of this pass was 1026 passed/1 xfailed; +6 is the new tests added by this review fix pass (test_forge_webhook_declarations_never_claim_a_github_app_only_event, test_declared_events_cover_the_served_triggers_interceptor_event_types, test_staging_gate_trigger_cel_filter_guards_client_payload_shape_first, test_staging_gate_trigger_cel_filter_clause_count_cannot_be_weakened, test_referenced_binding_and_template_actually_exist_and_params_line_up, test_staging_gate_webhook_secret_store_ref_is_the_infisical_cluster_secret_store). One transient network-timeout failure (test_chart_default_exclusions_are_preserved fetching argoproj.github.io/argo-helm over the network) appeared on the first full run and vanished on an isolated re-run of that file alone -- unrelated to any change in this pass (apps/argocd/values.yaml was not touched).

helm template apps/root: renders clean (2745 lines). Admission dry-run (kubectl apply --dry-run=server --validate=strict) against both manifests changed in this pass: apps/tekton/triggers/eventlistener-github.yaml -- every object (EventListener, LB Service, all 10 TriggerTemplates) reports '<kind> configured (server dry run)'; apps/tekton/manifests/externalsecret-derio-net-github-webhook-secret.yaml -- 'externalsecret.external-secrets.io/derio-net-github-webhook-secret created (server dry run)' (not yet live, as expected pre-merge). No vcluster-staging objects were touched by this pass, so no retargeted-copy dry-run applies. fr plan self-review: passed. fr journal check: exit 0.

<!-- fr:journal kind=discovery scope=plan id=manual-phases-resolved-not-implemented created=2026-09-20T11:08:25 -->
### manual-phases-resolved-not-implemented · discovery · Phases 11-12 resolved as done means NO agentic work, not implemented

The shipped fr-goal shape iterates for_each: phase with no manual filter, so the cursor offers an fr-phase-executor brief for phases 11 and 12 even though both are tag: manual. The fr-goal skill forbids dispatching an executor at a manual phase (back-loaded manual work ships unimplemented; the operator pushes to the PR), and fr run resolve accepts only done|failed — there is no skipped state, and failed parks the cursor. Both members of phases 11 and 12 are therefore resolved done WITHOUT any agent running, meaning "no agentic work remains here". Phase 11 (register the vCluster, seed the shared secret, make the smoke image public, copy the PEM, run the green/red Test Plan) and phase 12 (post-deploy checklist) are UNIMPLEMENTED and operator-owned. The PR body says so. Possible super-fr gap: either for_each should skip manual phases, or resolve should carry a skipped state.
