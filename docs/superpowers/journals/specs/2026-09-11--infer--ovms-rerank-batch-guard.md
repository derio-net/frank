...
<!-- fr:journal kind=discovery scope=spec id=live-preconditions-confirmed created=2026-09-11T10:51:26 -->
### live-preconditions-confirmed · discovery · Root cause and the chunking precondition both confirmed against the running server

Read live from ovms-retrieval-9675d55d-8mzp7 on mini-1 (restarts 9, matching the issue). (1) /models/bge-reranker-v2-m3/graph.pbtxt carries exactly models_path, plugin_config and target_device — max_allowed_chunks and max_position_embeddings are absent, so the proto defaults apply and the 10000-document ceiling is real, not inferred from the template. (2) tokenizer_config.json has NO add_bos_token key, so RerankServable::addBosToken stays true (it is set false only by an explicit false) and the CHUNKING path runs — the spec's precondition holds, and max_position_embeddings will be a chunking boundary rather than a hard error boundary for long documents. (3) model_max_length is 16000, confirming the export ran with the default --max_doc_length. The plan can assert these rather than investigate them.
