...
<!-- fr:journal kind=discovery scope=spec id=x2 created=2026-09-20T00:12:50 -->
### x2 · discovery · Correction to x1: the fr acceptance add defect is already filed as super-fr#433 / #470 — do not file

Checked before filing, per the repo's own rule about duplicates. super-fr#433 ('appends rows at the wrong indentation and rolls back on a column-0 matrix') and super-fr#470 ('rolls back on a matrix whose rows are not indented under rows:') are both OPEN and both describe exactly this. Frank's matrix.yaml has its rows at column 0 under 'rows:', which is the named trigger. x1's guess that the trailing folded scalar was the cause is therefore superseded — the cause is the matrix's indentation style, and the trailing block scalar is only where the misplaced insert happened to land. The hand-append workaround stands and needs no upstream action from this PR.
