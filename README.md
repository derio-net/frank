...
| Alert Agent | alert-agent (multi-agent-shell) | Autonomous `claude` agent (cloud brain, no local-inference dependency): daily digest + traffic-surge + Grafana-alert triage + inbound Telegram Q&A. Deterministic `frank-facts` CLI for facts; HTTP-only, no kube credential. Runs the self-updating **native** `claude` build (PV-resident, memory-safe install) with harness-agnostic `AGENTS.md`/`CLAUDE.md` instructions. Replaces the retired ai-alert-helper |
