...
**When to skip steps:**
- Fix/extension plans: skip blog posts (update the existing layer's posts instead). New gotchas: one-liner in `.claude/rules/frank-gotchas.md` (or `hop-gotchas.md`), full prose in `docs/runbooks/frank-gotchas-archive.md` under the same section heading.
