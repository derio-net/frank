...
5. **Update gotchas** — If the fix reveals a non-obvious pattern, add a one-liner to `.claude/rules/frank-gotchas.md` (or `hop-gotchas.md`) and the full prose / recovery commands to the matching per-topic file under `docs/runbooks/frank-gotchas/<topic>.md` (see that dir's `README.md` for the topic→file map). Hot file is auto-loaded into every session; per-topic files are not (read on demand).
