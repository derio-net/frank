...
#    open `fr:ready` GitHub issue is the nag — first fire creates it;
...
find_open_cleanup_issue() {
  ...
  gh issue list --repo "$REPO_SLUG" --state open --label fr:ready \
...
EOF
  ...
  # fr (v3) has no `issue create` subcommand (the old `vk issue create`
  # was removed in the super-fr split) — create the cleanup-nag issue
  # directly via `gh`. The issue carries the `fr:ready` lifecycle label so
  # the bridge can pick it up like any other agentic ticket.
  if [[ -n "${DRY_RUN:-}" ]]; then
    log "  DRY_RUN: would gh issue create --repo $REPO_SLUG --title \"$CLEANUP_TITLE\" --label fr:ready"
    return 0
  fi
  printf '%s\n' "$body" | gh issue create --repo "$REPO_SLUG" \
    --title "$CLEANUP_TITLE" --body-file - --label fr:ready >> "$LOG" 2>&1 \
