...
5. **Blog** — Use `/blog-craft:blog-post <series> <NN> <slug> "<title>"`. It scaffolds the page bundle, generates the cover, and runs `/blog-craft:media` for any media markers. The series overview lists the new post **automatically**: both `building/00-overview` and `operating/00-overview` render their index via the `{{< series-index >}}` shortcode (page-derived from the posts that exist — no append, no markers, always in sync), so there is **no overview edit step**. Then **add the new layer to `blog/data/roadmap.yaml`** (data-driven roadmap; not the old `cluster-roadmap.html` shortcode). If the post introduces a new technology, update the hand-curated **Technology → Capability Map** on `building/00-overview` by hand — that table is not page-derived
