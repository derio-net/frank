...
- LiteLLM-backed agents (`opencode_local` + `hermes_local`): opencode needs `litellm/<alias>` model shape; hermes v0.10.0 uses `--provider ollama-cloud --model <alias>` (no `inference.chain`); `HERMES_HOME` must be a writable PVC path; `hermes_local` adapter's `VALID_PROVIDERS` whitelist drops `ollama-cloud`, so `/paperclip/agent-bin/bin/hermes` is a wrapper installed by the hermes-init initContainer that injects `--provider ollama-cloud` — don't set `provider` or `extraArgs` from the UI — see `paperclip-ruflo.md#litellm-backed-agents`.
