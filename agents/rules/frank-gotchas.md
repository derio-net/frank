...
- Longhorn **v1.11.0 instance-manager leaks anonymous Go heap** (~0.9 GiB/day on busy nodes, unbounded; upstream proxy-connection leak, longhorn#12575, fixed v1.11.1+) — when it exhausts a node the kernel reclaim-thrashes instead of OOM-killing: kubelet/CRI wedge silently, node NotReady, NO process dies (took out raspi-1 2026-06-04; mini-1/gpu-1 carry 48/72 GiB leaked). Graceful `talosctl reboot` then wedges in `stopAllPods` (needs the dead CRI) — go straight to physical power-cycle (safe: immutable OS, journaled EPHEMERAL). IM restart resets the leak but kills live engines — drain first, or bump the chart (1.11.0→1.11.2) and live-upgrade engines so old IMs retire. Replica scheduling is disabled on raspi-1/raspi-2 (manual op `stor-longhorn-disable-pi-replica-scheduling`); `layer-1-node-memory-headroom` alert (<1GiB absolute, NOT a ratio) is the early warning. Full incident: `docs/investigations/2026-06-04--stor--raspi-1-memory-wedge-incident.md`.
