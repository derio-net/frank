...
- **`CI_AUTHORITY` is TWO variables, one per forge — not one.** `vars.CI_AUTHORITY` resolves in the forge running the workflow, and Gitea/GitHub have separate Actions-variable namespaces; flipping only Gitea leaves GitHub's unset → defaults to `github` → **BOTH sides run**. "Flip one variable, not five repos" is true per forge, not overall. Gitea half is API-settable (admin creds in `gitea-secrets`, ns `gitea`; `tekton-bot` lacks `read:organization`); GitHub half needs org-owner `admin:org` = operator-only. Flipped to `gitea` 2026-07-22 (Gitea half; GitHub half outstanding) because the GitHub minutes tier was exhausted, making `CI_AUTHORITY=github` mean nothing was authoritative. Manual op `cicd-stoa-ci-authority-cutover`; full prose: `tekton.md`.
